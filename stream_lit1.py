...
if st.session_state.page == "Home":
    ...
    st.markdown("""
        <style>pip install --upgrade streamlit
        .home-title {
            text-align: center;
            font-size: 3rem;
            font-weight: bold;
...
# Case Study Insights
    ...
    if case_option == "Decoding Transaction Dynamics":
        ...
        if not df_line.empty and "TotalCount" in df_line.columns:
         ...
         st.plotly_chart(px.line(df_line, x="Years", y="Count", markers=True, title="Insurance Growth Over Time", color_discrete_sequence=["#AC1D7F"]))
        else:
            st.warning("No data available .")
        ...
        st.plotly_chart(px.scatter(df_yearwise, x="Years", y="Total", title="Transaction Value Over Years", color_discrete_sequence=["#2404C0"]))
